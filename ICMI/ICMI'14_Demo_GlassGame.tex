<<<<<<< HEAD:SIGCHI2013LaTeX/GlassGameUISTDemo.tex
...
According to previous work\cite{headvideo,tele,robot,viewport}, using head orientation as a Viewport Control is undoubtedly intuitive (see Figure~\ref{fig:figure}.A).%However, we can't confirm whether using head orientation is really suitable for users to aim the target.
Therefore, in this work, we only explore the remaining control problems on Aim Control, Fire Control, and Move Control. 
First, we supposed 3 different aim control schemes to test which would be the most favorite control for users. 
Three different Aim Control schemes are listing below:
...
>>>>>>> 09144a01ddad44871bc92053d2123cb5e45a04a5:ICMI/ICMI'14_Demo_GlassGame.tex
